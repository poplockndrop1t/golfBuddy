  ...
  function addNewClub() {
    if (newClub.clubType && newClub.brand) {
      if (newClub.category !== "irons") {
        ...
        if (newClub.type === "3p") return createNewClub(newClub.category, newClub.clubType, newClub.brand, 8);
        if (newClub.type === "4a") return createNewClub(newClub.category, newClub.clubType, newClub.brand, 8);
        if (newClub.type === "4p") return createNewClub(newClub.category, newClub.clubType, newClub.brand, 7);
        if (newClub.type === "5a") return createNewClub(newClub.category, newClub.clubType, newClub.brand, 7);
        if (newClub.type === "5p") return createNewClub(newClub.category, newClub.clubType, newClub.brand, 6);
      }
    }
  };
